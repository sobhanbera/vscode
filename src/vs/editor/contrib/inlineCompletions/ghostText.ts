...
class PositionOffsetTransformer {
	private readonly lineBreakOffsetByLineIdx: number[];

	constructor(text: string) {
		this.lineBreakOffsetByLineIdx = [];
		let lineNumber = 1;
		for (let i = 0; i < text.length; i++) {
			if (text.charAt(i) === '\n') {
				this.lineBreakOffsetByLineIdx.push(i + 1);
				lineNumber++;
			}
		}
	}
	...
	getOffset(position: Position): number {
		const offset = position.lineNumber === 1 ? 0 : this.lineBreakOffsetByLineIdx[position.lineNumber - 2];
